...
const CreateConnectionDialog = ({ open, onClose, onConnectionCreated }) => {
  ...
  const testConnection = async () => {
    try {
      ...
      const response = await singleServerConnectionsAPI.test(testData);
  ...
  const saveConnection = async () => {
    try {
      ...
      await singleServerConnectionsAPI.create(connectionData);
      onConnectionCreated();
      handleClose();
    } catch (err) {
      setError(err.message);
          ...
          <Grid container spacing={2}>
            <Grid item xs={12}>
              <TextField
                ...
                required
                helperText="A unique name to identify this single-server connection"
              />
            </Grid>
            <Grid item xs={12}>
